...
export default function SearchBooks(){
 ...
 let [searchedResults,setSearchedResults]=useState(null);
    ...
    <ToastContainer/>
        ...
        required={true} 
        onInput={e => {setSearchInput(e.target.value);e.target.value===""?setSearchedResults(null):null}} 
        value={searchInput} 
        className="border border-white p-2 w-full rounded-lg outline-0 rounded-r-none" type="text" />
        <button className="bg-[#003153] p-2 border-2 border-[#003153] rounded-lg rounded-l-none" type="submit">Search</button>
        </div>
        {<div>
            <ul className="">
                 {searchedResults?.map(
                ...
                <p className="border border-white p-4">No Results Found</p>:null}
            </ul>
        </div>}
    </form>
    </>
